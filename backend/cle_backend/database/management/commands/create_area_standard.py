...
class Command(BaseCommand):
    help = 'Create the group area_standard and assign permissions'

    def handle(self, *args, **options):
        # Crear Grupo 'area_standard'
        area_standard_group, created = Group.objects.get_or_create(name='area_standard')

        # Asignar permisos al grupo 'area_standard'
        models_permissions = {
        ...
        for model, permissions in models_permissions.items():
            ...
            area_standard_group.permissions.add(*perms)

        self.stdout.write(self.style.SUCCESS('Successfully created the area_standard group and assigned permissions'))
